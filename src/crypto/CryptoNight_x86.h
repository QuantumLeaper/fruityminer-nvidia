...
static inline void cryptonight_monero_tweak(uint64_t* mem_out, const uint8_t* l, uint64_t idx, __m128i ax0, __m128i bx0, __m128i bx1, __m128i& cx)
{
    if (BASE == xmrig::VARIANT_2) {
        VARIANT2_SHUFFLE(l, idx, ax0, bx0, bx1, cx, (VARIANT == xmrig::VARIANT_UPX2 ? 1 : 0));
    ...
    for (size_t i = 0; i < ITERATIONS; i++) {
        ...
        if (BASE == xmrig::VARIANT_2) {
            if (VARIANT == xmrig::VARIANT_4) {
                ...
                VARIANT2_SHUFFLE2(l0, idx0 & MASK, ax0, bx0, bx1, hi, lo, (VARIANT == xmrig::VARIANT_UPX2 ? 1 : 0));
            }
        }
...
extern xmrig::CpuThread::cn_mainloop_fun        cn_upx2_mainloop_ivybridge_asm;
extern xmrig::CpuThread::cn_mainloop_fun        cn_upx2_mainloop_ryzen_asm;
extern xmrig::CpuThread::cn_mainloop_fun        cn_upx2_mainloop_bulldozer_asm;
extern xmrig::CpuThread::cn_mainloop_fun        cn_upx2_double_mainloop_sandybridge_asm;
...
inline void cryptonight_single_hash_asm(const uint8_t *__restrict__ input, size_t size, uint8_t *__restrict__ output, cryptonight_ctx **__restrict__ ctx, uint64_t height)
{
    ...
    else if (VARIANT == xmrig::VARIANT_UPX2) {
    ...
    for (size_t i = 0; i < ITERATIONS; i++) {
        ...
        if (BASE == xmrig::VARIANT_2) {
            if (VARIANT == xmrig::VARIANT_4) {
                ...
                VARIANT2_SHUFFLE2(l0, idx0 & MASK, ax0, bx00, bx01, hi, lo, (VARIANT == xmrig::VARIANT_UPX2 ? 1 : 0));
            }
        }
        ...
        if (BASE == xmrig::VARIANT_2) {
            if (VARIANT == xmrig::VARIANT_4) {
                ...
                VARIANT2_SHUFFLE2(l1, idx1 & MASK, ax1, bx10, bx11, hi, lo, (VARIANT == xmrig::VARIANT_UPX2 ? 1 : 0));
            }
        }
...
#define CN_STEP4(part, a, b0, b1, c, l, mc, ptr, idx)   \
    ...
    if (BASE == xmrig::VARIANT_2) {                     \
        if (VARIANT == xmrig::VARIANT_4) { \
            ...
            VARIANT2_SHUFFLE2(l, idx & MASK, a, b0, b1, hi, lo, (VARIANT == xmrig::VARIANT_UPX2 ? 1 : 0)); \
        } \
    }                                                   \
    if (VARIANT == xmrig::VARIANT_4) { \
